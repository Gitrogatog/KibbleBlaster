...
public class SpatialHash<T> where T : unmanaged, System.IEquatable<T>
{
    ...
    protected readonly Dictionary<T, AABB> IDBoxLookup = new Dictionary<T, AABB>();
    ...
    public virtual void Insert(T id, AABB rectangle)
    {
        var relativeX = (int)rectangle.X - X;
        var relativeY = (int)rectangle.Y - Y;
    ...
    /// </summary>
    public RetrieveEnumerator Retrieve(T id, AABB rectangle)
    {
        var relativeX = (int)rectangle.X - X;
        var relativeY = (int)rectangle.Y - Y;
        var rowRangeStart = Math.Clamp(relativeX / CellSize, 0, RowCount - 1);
        var rowRangeEnd = Math.Clamp((relativeX + (int)rectangle.Width) / CellSize, 0, RowCount - 1);
        var columnRangeStart = Math.Clamp(relativeY / CellSize, 0, ColumnCount - 1);
        var columnRangeEnd = Math.Clamp((relativeY + (int)rectangle.Height) / CellSize, 0, ColumnCount - 1);
    ...
    /// <returns></returns>
    public RetrieveEnumerator Retrieve(AABB rectangle)
    {
        var relativeX = (int)rectangle.X - X;
        var relativeY = (int)rectangle.Y - Y;
        var rowRangeStart = Math.Clamp(relativeX / CellSize, 0, RowCount - 1);
